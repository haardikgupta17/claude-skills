...
Search repos in PRIORITY ORDER (repo #1 first) to find the exact files that need to be modified. This is critical — the developer should know EXACTLY where to start coding and in WHICH repo.

**TOKEN-EFFICIENT SEQUENTIAL STRATEGY:**

1. **Start with repo #1** (highest priority). Search using Glob and Grep:
   - Glob for files matching keywords from the ticket (table names, pipeline names, module names)
   - Grep for specific identifiers: table names, column names, function names, error messages, DAG IDs
   - Read the top 3 most relevant files (limit to 100 lines each) to understand current implementation

2. **If relevant files found in repo #1 → STOP searching other repos** unless the ticket clearly requires cross-repo changes (e.g. "add column to datamart AND update the DAG").

3. **If NOT found in repo #1 → move to repo #2**, repeat the same search. Continue down the priority list.

4. **Only search the lowest-priority repos** if nothing was found above OR the ticket explicitly references them.

5. **For each file you identify, note:**
   - The repo name and file path (relative to repo root), e.g. `[servicedatamart] ddl/edw/storedprocedures/MyProc.sql`
   - Which line/section needs changes
   - What kind of change is needed (add, modify, delete)
   - A brief description of the change

6. **Cross-repo awareness** — Only if the ticket clearly requires changes in multiple repos (e.g. add a column in the datamart SQL AND update the DAG that loads it). Don't search extra repos "just in case."
...
**IMPORTANT: Comments should be restricted to "Developers" role** so only the dev team sees the AI-generated briefs — not customers, PMs, or external stakeholders.

**DO NOT use `mcp__jira__AddJiraComment`** — it does NOT support visibility restrictions. Comments post as "All Users."

**ALWAYS use curl + JIRA_PAT to post restricted comments:**
```bash
...
**Setup:** Create a Jira Personal Access Token and add to your shell profile:
```bash
export JIRA_PAT="your-token-here"
```

**If `JIRA_PAT` is not set** (curl returns 401), fall back to `mcp__jira__AddJiraComment` and print:
```
⚠ Jira comment posted as "All Users" — JIRA_PAT not configured.
  → Set: export JIRA_PAT=<your-token> in ~/.zshrc for restricted comments.
```
